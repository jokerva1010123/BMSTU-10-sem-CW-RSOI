...
// func (r SqlRepository) Get(ctx context.Context, id int) (income Income, err error) {
// 	err = r.db.With(ctx).Select().Model(id, &income)

// 	// row := r.db.DB().DB().QueryRow("SELECT scope_id, author_id, CreatedAt, UpdatedAt FROM $1 where ID = $2;", income.TableName(), id)
...
// func (r SqlRepository) Create(ctx context.Context, income *Income) error {
// 	return r.db.With(ctx).Model(income).Insert()
// }

// // Update saves the changes to an album in the database.
// func (r SqlRepository) Update(ctx context.Context, income Income) error {
// 	return r.db.With(ctx).Model(&income).Update()
// }
...
// 	income, err := r.Get(ctx, id)
// 	if err != nil {
// 		return err
// 	}
// 	return r.db.With(ctx).Model(&income).Delete()
// }

// // Count returns the number of the album records in the database.
// func (r SqlRepository) Count(ctx context.Context) (int, error) {
// 	var count int
// 	err := r.db.With(ctx).Select("COUNT(*)").From("incomes").Row(&count)
// 	return count, err
// }

// // Query retrieves the album records with the specified offset and limit from the database.
// func (r SqlRepository) Query(ctx context.Context, offset, limit int) (incomes []Income, err error) {
// 	err = r.db.With(ctx).
// 		Select().
// 		OrderBy("id").
// 		Offset(int64(offset)).
